...
import {BoatActionTypes, CreateBoat, CreateBoatFailure, CreateBoatSuccess} from '../actions/boat.actions';
  ...
  getSnapshotsLocalStorage$: Observable<Action> = this.actions$
    .pipe(
      ...
      map(() => {
        ...
        const snapshots = snapshotsJSON.map(x => new Snapshot(x.time, x.time_trial_id));
